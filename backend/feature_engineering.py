...
class FeatureBuffer:
    ...
    def _window(self, seconds: int, now_ts: float) -> Tuple[_Sample, ...]:
        cutoff = now_ts - seconds
        return tuple(s for s in self._samples if s.ts >= cutoff)

    def build_features(self, raw: Dict) -> Dict[str, float]:
        now_ts = _now_ts()
        ...
        self._samples.append(
            _Sample(ts=now_ts, distance_cm=distance_cm, rain_analog=rain_analog, float_status=float_status)
        )

        # --- Rolling distance features (3 minutes) ---
        win_3m = self._window(3 * 60, now_ts)
        ...
        win_5m = self._window(5 * 60, now_ts)
        ...
        win_30m = self._window(30 * 60, now_ts)
        ...
        if rain_present and self._rain_start_ts is None:
            self._rain_start_ts = now_ts
        if not rain_present:
            self._rain_start_ts = None
        time_since_rain_start = 0.0
        if self._rain_start_ts is not None:
            time_since_rain_start = float((now_ts - self._rain_start_ts) / 60.0)
